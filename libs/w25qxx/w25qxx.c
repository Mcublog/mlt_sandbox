...
{
    uint8_t status = 0;
    if (w25qxx.ID == AT45DB641)
    {
      HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_RESET);
      W25qxx_Spi(0xD7);
      w25qxx.StatusRegister1 = W25qxx_Spi(W25QXX_DUMMY_BYTE);
      w25qxx.StatusRegister2 = W25qxx_Spi(W25QXX_DUMMY_BYTE);
      w25qxx.StatusRegister3 = 0;
      if      (SelectStatusRegister_1_2_3 == 1) status = w25qxx.StatusRegister1;
      else if (SelectStatusRegister_1_2_3 == 2) status = w25qxx.StatusRegister2;
      else status = w25qxx.StatusRegister3;
...
//###################################################################################################################
uint8_t W25qxx_ReadSectoreLockDownRegister(uint32_t addr)
{
    uint8_t status = 0;
    ...
    if (w25qxx.ID == AT45DB641)
    {
      ...
      //for (uint32_t i = 0; i < 32; i++) printf("Sector %d: 0x%x\n", i, W25qxx_Spi(W25QXX_DUMMY_BYTE));
      for (uint32_t i = 0; i < 32; i++) W25qxx_Spi(W25QXX_DUMMY_BYTE);
...
void W25qxx_WaitForWriteEnd(void)
{
    ...
    do
    {
      if (w25qxx.ID == AT45DB641)
      {
        ...
        w25qxx.StatusRegister1 = W25qxx_Spi(W25QXX_DUMMY_BYTE);
        if (w25qxx.ID == AT45DB641) w25qxx.StatusRegister2 = W25qxx_Spi(W25QXX_DUMMY_BYTE);
...
//        printf("w25qxx.StatusRegister2: 0x%x\n", w25qxx.StatusRegister2);
    ...
    W25qxx_Delay(5);
    } while ((w25qxx.StatusRegister1 & rdymask) != rdymask);
    if (w25qxx.ID != AT45DB641)HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_SET);
}
//###################################################################################################################
bool    W25qxx_Init(void)
{
...
#endif
        break;
    case 0x2800: //    at45db641
        w25qxx.ID = AT45DB641;
        w25qxx.BlockCount = 32;
#if (_W25QXX_DEBUG == 1)
        printf("w25qxx Chip: at45db641\r\n");
#endif
        break;
    default:
#if (_W25QXX_DEBUG == 1)
    ...
    if (w25qxx.ID == AT45DB641)
    {
        ...
        w25qxx.SectorCount = w25qxx.BlockCount * 128;
        w25qxx.PageCount = (w25qxx.SectorCount * w25qxx.SectorSize) / w25qxx.PageSize;
        w25qxx.BlockSize = w25qxx.SectorSize * 128;
    ...
    if (w25qxx.ID == AT45DB641 && (!(state & 0x1))) //Swith to 256 bytes page
    {
        HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_RESET);
        W25qxx_Delay(1);
        W25qxx_Spi(0x3D);
...
#endif
    if (w25qxx.ID != AT45DB641) W25qxx_WriteEnable();
    HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_RESET);
    W25qxx_Delay(1);
    W25qxx_Spi(0xC7);
    if (w25qxx.ID == AT45DB641)
...
void W25qxx_EraseSector(uint32_t SectorAddr)
{
    ...
    HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_RESET);
    W25qxx_Spi(0x20);
    if (w25qxx.ID >= W25Q256 && w25qxx.ID != AT45DB641)
...
void W25qxx_ErasePage(uint32_t PageAddr)
{
    ...
    {
      //PageAddr = (PageAddr >> 8);
      PageAddr = (PageAddr * w25qxx.PageSize);
      cmd = 0x81;
    }
    else
    {
    ...
    //PageAddr = (PageAddr * w25qxx.PageSize);
    if (w25qxx.ID >= W25Q256 && w25qxx.ID != AT45DB641)
        W25qxx_Spi((PageAddr & 0xFF000000) >> 24);
...
void W25qxx_ReadPage(uint8_t *pBuffer, uint32_t Page_Address, uint32_t OffsetInByte, uint32_t NumByteToRead_up_to_PageSize)
{
    ...
    if (w25qxx.ID == AT45DB641)
    {
      W25qxx_Spi(W25QXX_DUMMY_BYTE);//Dummy
//      W25qxx_Spi(W25QXX_DUMMY_BYTE);
//      W25qxx_Spi(W25QXX_DUMMY_BYTE);
//      W25qxx_Spi(W25QXX_DUMMY_BYTE);
    }
...
//###################################################################################################################
void W25qxx_GoToDeepSleep(void)
{
    HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_RESET);
    W25qxx_Spi(0xB9);
    HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_SET);
...
void W25qxx_ResumeFromDeepSleep(void)
{
    HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_RESET);
    W25qxx_Spi(0xAB);
    HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_SET);
...
void W25qxx_GoToUltraDeepSleep(void)
{
    HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_RESET);
    W25qxx_Spi(0x79);
    HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_SET);
...
void W25qxx_ResumeUltraFromDeepSleep(void)
{
    HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_RESET);
    HAL_Delay(1);
    HAL_GPIO_WritePin(_W25QXX_CS_GPIO, _W25QXX_CS_PIN, GPIO_PIN_SET);
